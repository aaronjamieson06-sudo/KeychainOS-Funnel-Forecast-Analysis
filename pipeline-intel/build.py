...
<div class="page" id="page-1">
  ...
  <div class="g2" style="margin-top:22px;">
    ...
    <div class="card">
      ...
      <div class="mrow"><span class="mrl">Projected Closes / Month</span><span class="mrv by" id="s2-proj">25</span></div>
...
<div class="page" id="page-2">
  ...
  <div style="display:grid;grid-template-columns:1fr 290px;gap:20px;margin-top:22px;align-items:start;">
    <div>
      ...
      <div class="frow"><div class="ftrack" style="border-color:rgba(245,208,0,0.5);background:rgba(245,208,0,0.04);"><div class="ffill" id="f-f5" style="width:9.5%;opacity:0.35;"></div><span class="fbl" style="font-weight:600;color:var(--text);">Projected Close Quota</span><span class="fbv by" id="f-proj">25</span></div></div>
...
<div class="page" id="page-3">
  ...
  <div class="sgrid">
    <div class="sc">
      ...
      <div class="srow"><span class="srl">Projected close quota</span><span class="srv by" id="s4-p1">25</span></div>
    ...
    <div class="sc">
      ...
      <div class="srow"><span class="srl">New projected closes</span><span class="srv by" id="s4-p2">25</span></div>
    ...
    <div class="sc">
      ...
      <div class="srow"><span class="srl">New projected closes</span><span class="srv by" id="s4-p3">25</span></div>
...
function model(cr, acv, leads) {{
  ...
  // M3: cw from fresh pipeline + resolved M2 carries + M1 residual
  const m2Carry = proj - cw;                    // same carry rate as M1 = 5
  const m2Resolved = Math.round(m2Carry * 0.8); // 4
  const m1Residual = Math.max(1, Math.round(carry * 0.1)); // 1
  const m3cw = cw + m2Resolved + m1Residual;    // 20 + 4 + 1 = 25
  ...
  function sc(crMult) {{
    ...
    const sM2carry = sProj - sCw;
    const sM2r = Math.round(sM2carry * 0.8);
    const sM1res = Math.max(1, Math.round(sCarry * 0.1));
    const sM3cw = sCw + sM2r + sM1res;
    return {{
...
function buildRepTable(d) {{
  const rows=[
    {{r:6, t:4, a:(d.cw/6).toFixed(1), s:'efficient'}},
    {{r:8, t:3, a:(d.cw/8).toFixed(1), s:'optimal'}},
    {{r:10,t:2, a:(d.cw/10).toFixed(1),s:'diminishing'}},
    {{r:11,t:2, a:(d.cw/11).toFixed(1),s:'constrained'}},
  ];
  const badges={{
    efficient:`<span style="background:#F0FBF6;color:#1A9E5F;font-size:10px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;padding:3px 8px;border-radius:4px;">Efficient</span>`,
    optimal:`<span style="background:#0A0A0A;color:#F5D000;font-size:10px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;padding:3px 8px;border-radius:4px;">Optimal</span>`,
    diminishing:`<span style="background:#FFF4E8;color:#B05010;font-size:10px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;padding:3px 8px;border-radius:4px;">Diminishing</span>`,
    constrained:`<span style="background:#FFF0F0;color:#E03030;font-size:10px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;padding:3px 8px;border-radius:4px;">Lead-constrained</span>`,
  }};
  ...
  tb.innerHTML=rows.map(r=>`
    <tr>
      <td>${{r.r}} Reps</td>
      <td>${{r.t}} / mo</td>
      <td style="font-size:18px;font-weight:800;letter-spacing:-0.3px;color:${{parseFloat(r.a)<2?'#E03030':parseFloat(r.a)<2.5?'#B05010':'inherit'}}">${{r.a}}</td>
      <td>~${{d.cw}}</td>
      <td>${{fmtM(d.arrM1)}}</td>
      <td>${{badges[r.s]}}</td>
    </tr>
